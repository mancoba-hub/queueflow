...
using Queue.Flow.Application.Features.Products.Commands.CreateProduct;
using MediatR;
...
public class ProductsController : ControllerBase
{
        ...
        [FromQuery] string? category = null)
    {
        ...
        return await Task.FromResult<ActionResult>(results);
    }
    ...
    public async Task<ActionResult<ProductDto>> GetProduct(Guid id)
    {
        ...
        return await Task.FromResult<ActionResult>(NotFound(new { message = $"Product with ID {id} not found" }));
    }
    ...
    public async Task<ActionResult<ProductDto>> CreateProduct([FromBody] CreateProductCommand request)
    {
        try
        {
            ...
            return await Task.FromResult<ActionResult>(CreatedAtAction(nameof(GetProduct), new { id = product.Id }, product));
        }
        catch (InvalidOperationException ex) when (ex.Message.Contains("already exists"))
        {
            _logger.LogWarning("Product creation attempt with existing SKU: {Sku}", request.Sku);
            return await Task.FromResult<ActionResult>(Conflict(new { message = "Product with this SKU already exists" }));
        }
    }
    ...
    public async Task<ActionResult<ProductDto>> UpdateProduct(Guid id, [FromBody] UpdateProductDto request)
    {
        ...
        return await Task.FromResult<ActionResult>(StatusCode(StatusCodes.Status501NotImplemented,
